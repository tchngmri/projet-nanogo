...
let rec islvalue x = true
 (* fonction qui détermine si c'est une l value... voir le typage *)
...
| Tmany (x::q) -> "??????"
| Twild -> "??????" (*A FAIRE*)
let errortype loc exp real = error loc ("this expression has type"^(typetostr real)^" but is expected to have type"^(typetostr exp))
...
let phase1 = function
  | PDstruct ({ ps_name = { id = id; loc = loc }} as s) -> if Hashtbl.mem struct_env id then 
    ...
    else () (*Hashtbl.add struct_env id (pstruct_to_struct s)*)
    (*faire une fonction pstrust -> struct*)
  | PDfunction _ -> ()
...
let phase2 = function
  | PDfunction ({ pf_name={id; loc}; pf_params=pl; pf_typ=tyl; } as f) -> (* TODO *)
      (if f.pf_name.id == "main" then 
        ...
        else (if (f.pf_typ != []) then error f.pf_name.loc "main function can't return anything" else found_main:=true)))
...
let file ~debug:b (imp, dl) =
  debug := b;
  fmt_imported := imp; (*??? jcomprends pas comment on check si fmt est immporté*)
  List.iter phase1 dl;
  List.iter phase2 dl;
